...
t1 = Theta1(:, 2 : end);
t2 = Theta2(:, 2 : end);
Reg_JPart = ( sum( sum( t1 .^ 2)) + sum( sum( t2 .^ 2)) ) * ( lambda / (2*m) );
...
for i = 1:m
	...
	a1 = [1; X(i,:)']; % input layer 
	a2 = [1 ; sigmoid( Theta1*a1 ) ]; % layer 2, hidden
	a3 = sigmoid(Theta2*a2); % layer 3, Output

	Y_ex = ( [1:num_labels] == y(i) )'; 
    % deltas :
	del3 = a3 - Y_ex; % each output layer
	del2 = ((Theta2')*del3) .* [1;sigmoidGradient(Theta1*a1)];
	del2 = del2(2:end); % exclude bais
